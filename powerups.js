...
function randomPowerType(){
    const types = ["weapon", "speed", "heal", "shield", "bomb"];
    return types[Math.floor(Math.random() * types.length)];
}

function spawnPowerUp(x, y){
    powerUps.push(new PowerUp(x, y, randomPowerType()));
}
...
function powerUpCollision(player, power){
    const dx = player.x - power.x;
    const dy = player.y - power.y;
    const distance = Math.sqrt(dx*dx + dy*dy);
    return (distance < player.radius + power.radius);
}

function updatePowerUps(player){
    powerUps.forEach(power => {
        power.update();
        if(powerUpCollision(player, power)){
