...
const heart = {
    ...
    draw(){
        ...
        drawHeart(this.x, this.y, this.radius);
        ctx.restore();
    }
};
heart.update();
...
function handleEnemyHeartCollisions(){
    enemies.forEach(enemy => {
        ...
        if(circleCollision(enemy.x, enemy.y, enemy.radius, heart.x, heart.y, heart.radius)){
            enemy.active = false;
            heart.lives--;
            showMessage("💔 El corazón fue golpeado");
            if(heart.lives <= 0) gameOver();
        }
    });
}
...
function handleBossCollisions(){
    ...
    player.bullets.forEach(bullet => {
        ...
        if(bulletBossCollision(bullet, boss)){
            bullet.active = false;
...
function updateLevel(){
    if(score >= 200 && level === 1) { level = 2; showMessage("❤️ Nivel 2"); }
    if(score >= 500 && level === 2) { level = 3; showMessage("💖 Nivel 3"); }
    if(score >= 900 && level === 3) { level = 4; }
    if(score >= 1400 && level === 4) { level = 5; }
    if(score >= 2000 && level === 5) { level = 6; }
    if(score >= 2800 && level === 6) { level = 7; }
    if(score >= 3800 && level === 7) { level = 8; }
    if(score >= 5000 && level === 8) { level = 9; }
    if(score >= 6500 && level === 9) { level = 10; showMessage("💀 JEFE FINAL"); }
}
...
function spawnEnemies(){
    ...
    let type = "small";
    if(level >= 2 && Math.random() < .4) type = "medium";
    if(level >= 4 && Math.random() < .25) type = "large";
    if(level >= 7 && Math.random() < .15) type = "giant";
...
function update(deltaTime){
    ...
    if(level >= 10 && !bossStarted){
        bossStarted = true;
        boss.start();
        showMessage("💀 JEFE FINAL 💀");
    }
    ...
    weaponElement.textContent = player.weaponLevel;
}

function drawBackground(){
    const gradient = ctx.createLinearGradient(0, 0, 0, canvas.height);
    gradient.addColorStop(0, "#060818");
    gradient.addColorStop(1, "#1d1238");
